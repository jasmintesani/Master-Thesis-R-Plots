...
data <-
  read.csv(
    'C:\\Users/Filippo/Desktop/October exp/Evo/CFUperOD595_Noncoated_3materials_MScThesisTesani2025/CFUperOD595_Noncoated_3materials_MScThesisTesani2025.csv'
  )
...
plot <- ggplot(summary_data_long,
  ...
  scale_y_continuous(
    limits = c(0, 2.2e+08),
    expand = c(0, 0),
    breaks = seq(0, 2.2e+08, by = 0.2e+08),
  ...
  labs(x = "\nTime (days)",
       y = expression(bold("CFU/bead/Biomass(" * OD[595] * ")")),
       fill = "Condition",
       title = "") +
  theme_minimal() +
  theme(
