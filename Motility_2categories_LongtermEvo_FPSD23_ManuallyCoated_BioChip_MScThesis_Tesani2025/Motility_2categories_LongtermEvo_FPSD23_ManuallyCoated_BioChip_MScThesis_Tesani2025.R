...
data <-
  read.csv(
    'C:\\Users/Filippo/Desktop/October exp/Evo/Motility_2categories_LongtermEvo_FPSD23_ManuallyCoated_BioChip_MScThesis_Tesani2025/Motility_2categories_LongtermEvo_FPSD23_ManuallyCoated_BioChip_MScThesis_Tesani2025.csv'
  )

data <- data[, grepl("ncb|mot", colnames(data), ignore.case = TRUE)]

unique_conditions <- c("ncb")
...
the_plot <- ggplot(data_long,
  ...
  scale_fill_manual(
    values = c("1" = "gray65",
               "2" = "gray35"),
    labels = c("Not motile", "Motile")
  ) +
  labs(x = "Time (days)",
       y = "",
       fill = "",
       title = "Noncoated beads") +
  theme_minimal() +
  theme(
