...
data <-
  read.csv(
    'C:\\Users/Filippo/Desktop/October exp/Evo/OD595_NoncoatedBeads_DaniaPlastMaterials_MScThesisTesani2025/OD595_NoncoatedBeads_DaniaPlastMaterials_MScThesisTesani2025.csv'
  )
...
for (j in seq(2, length(colnames(data)), by = 11)) {
  mb_index <- j
  pf43_index <- j + 9
...
condition_positions <- seq(-0.432, 0.432, length.out = 11)

# Convert to a named vector (example names)
names(condition_positions) <- unique_conditions
...
max_heights <- summary_data_long %>%
  group_by(day) %>%
  summarise(ypos = max(absorbance + sd) + 0.00)

offsets <- seq(0, 0.5, length.out = length(unique_conditions))

# Assign offsets based on the ordering in unique_conditions
star_data$yoffset <- offsets[match(star_data$col2, unique_conditions)]
...
write.csv(
  ...
  paste0(
    ...
    "/OD595_NoncoatedBeads_DaniaPlastMaterials_MScThesisTesani2025",
...
the_plot <- ggplot(summary_data_long,
                   aes(
                     x = factor(day),
                     y = absorbance,
                     fill = condition,
  ...
  #     xmin = x1,
  #     xmax = x2,
  #     y_position = ifelse(col1 == "mb", ypos + yoffset - 0.05, ypos + yoffset) + 0.05,
  #     group = col2,
  #     annotations = "***",
  #     tip_length = 0.01,
  #   ),
