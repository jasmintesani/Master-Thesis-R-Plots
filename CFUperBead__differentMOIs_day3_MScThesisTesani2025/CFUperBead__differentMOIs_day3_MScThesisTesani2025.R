...
df <- data_wide %>% rename(moi = variable)
...
summary_data_long <- df_means %>%
  left_join(df_sds,
            by = c("moi", "condition" = "condition_sd")) %>%
...
summary_data_long$moi <-
  gsub("x", "", summary_data_long$moi, ignore.case = TRUE)
...
plot <- ggplot(summary_data_long,
  ...
  scale_y_continuous(
    limits = c(0, 8.1e+07),
    expand = c(0, 0),
    breaks = seq(0, 8.1e+07, by = 0.5e+07),
