...
write_xlsx(summary_data,
           path = glue(
             ...
             "/Nexabiome_CocktailCoated_BioChip_OD595_bead&wells_MScThesisTesani2025",
             "/Nexabiome_CocktailCoated_BioChip_OD595_bead&wells_MScThesisTesani2025.xlsx"
           ),
           col_names = TRUE)
...
plot <- ggplot(summary_data_long,
  ...
  labs(x = "\nTime (days)",
       y = "OD at 590 nm\n",
       fill = "Condition",
       title = "Biofilm on wells surrounding the beads coated with a phage cocktail") +
  theme_minimal() +
  theme(
